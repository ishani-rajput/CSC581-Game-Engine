...
static inline void clampPlayerPosition(float &playerX, float &playerY, float w, float h)
{
    if (playerX < 0)
        playerX = 0;
    if (playerX + w > DESIGN_WIDTH)
        playerX = DESIGN_WIDTH - w;
    if (playerY > DESIGN_HEIGHT)
        playerY = DESIGN_HEIGHT - h;
}
...
int main(int, char **)
{
    ...
    SDL_Log("Sending disconnect message...");
    char disconnectMsg[256];
    snprintf(disconnectMsg, sizeof(disconnectMsg), "DISCONNECT %s", clientId.c_str());
    zmq_send(sock, disconnectMsg, strlen(disconnectMsg), 0);

    char ack[256];
    int ackSize = zmq_recv(sock, ack, sizeof(ack) - 1, 0);
    if (ackSize > 0)
    {
        ack[ackSize] = '\0';
        SDL_Log("Server acknowledged disconnect");
    }

    // Don't wait for response, server will just process it
    SDL_Log("Client %s disconnected", clientId.c_str());

    // Cleanup
    for (auto &kv : remotes)
        if (kv.second.entity)
            delete kv.second.entity;
    zmq_close(sock);
    zmq_ctx_destroy(ctx);
