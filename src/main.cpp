...
int main(int, char**){
    ...
    SDL_SetRenderVSync(renderer, 1); // smoother present timing
    // assets
    ...
    const int FRAME_COUNT=6, ANIM_MS=100; const float FRAME_W=texW/FRAME_COUNT, FRAME_H=texH;
    ...
    // Set character size - fixed for pixel mode, relative for proportional mode
    ...
    // ground
    SDL_FRect ground={0.f, 1080.f-120.f, 1920.f, 120.f};  // Fixed dimensions for pixel mode
    // pipes
    std::vector<PipePair> pipes; const float PIPE_SPEED=-450.f, PIPE_W=140.f, PIPE_GAP=280.f;
    const float PIPE_SPAWN_EVERY=1.4f; float spawnTimer=0.f;
    // timing/input
    Uint32 lastTicks=SDL_GetTicks(); bool running=true; SDL_Event ev; bool prevSpace=false; bool prevToggle=false;
    // window size tracking
    int winW=0, winH=0; SDL_GetWindowSize(window, &winW, &winH);
    
    // scaling
    Scaling::setMode(ScaleMode::Pixel);
    auto resetPipesOnly=[&](){ pipes.clear(); spawnTimer=0.f; };
    auto spawnPipe=[&](){
        // Use fixed base dimensions - Scaling::compute() will handle the scaling
    ...
    while(running){
        ...
        if (dtSec > 0.033f) dtSec = 0.033f; // cap ~33ms
        lastTicks=now;
        // animation
        if(now-lastAnimTick >= (Uint32)ANIM_MS){ currentFrame=(currentFrame+1)%FRAME_COUNT; lastAnimTick=now; }
        // jump
        bool spaceNow=Input::isKeyPressed(SDL_SCANCODE_SPACE);
        if(spaceNow && !prevSpace) skBody.vy=JUMP_VELOCITY; prevSpace=spaceNow;
        
        // toggle scaling mode (T key)
        ...
        // pipes
        spawnTimer+=dtSec; if(spawnTimer>=PIPE_SPAWN_EVERY){ spawnTimer-=PIPE_SPAWN_EVERY; spawnPipe(); }
        for(auto& p:pipes){ p.top.x+=PIPE_SPEED*dtSec; p.bottom.x+=PIPE_SPEED*dtSec; }
        pipes.erase(std::remove_if(pipes.begin(),pipes.end(),
            [&](PipePair& pp){ return (pp.top.x+pp.top.w)<-50.f; }), pipes.end());
        bool hit=false; for(auto& p:pipes){ if(aabbIntersect(skully,p.top)||aabbIntersect(skully,p.bottom)){ hit=true; break; } }
        ...
        // ground: tile platform.png across the width
